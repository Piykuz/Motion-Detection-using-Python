...
email_id = 'Reciver_Email'  #enter reciever Email
Path = os.path.abspath(os.getcwd())+r'\Recording'  #"D\\Programming\Python\Motion_Detection_using_Python\Recording\\"
...
def Email_Sender():
    ...
    try:
        ...
        # Authentication
        s.login("Sender_Email", "Password") #Sender email and password

        # message to be sent

        message = "Intruder ALERT ! Someone is Detected in your Room "

        s.sendmail("Sender_Email", email_id, message)
        s.quit()
    except:
        pass
    finally:
